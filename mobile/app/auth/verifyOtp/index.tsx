...
export default function VerifyOtpPage() {
  ...
  // Pastikan kita mengambil string, jika array ambil yang pertama
  ...
  // 2. JEBAKAN DEBUGGING (Cek Console Metro Bundler)
  useEffect(() => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      console.log("Status Backend:", response.status); // Debugging

      // LOGIC SUKSES
      if (response.data.success || response.status === 200) {
        setLoading(false); // Matikan loading dulu
        console.log("OTP Valid! Pindah ke Reset Password...");

        // --- NAVIGASI KE HALAMAN RESET PASSWORD ---
        // Kita bawa 'email' dan 'otpCode' ke halaman selanjutnya
        // Pastikan folder Anda bernama 'resetPassword' (Sesuai screenshot sebelumnya)

        router.push({
          pathname: "/auth/resetPassword",
          params: {
            email: email,
            otpCode: otp, // Penting: Bawa kode OTP ini
          },
        } as any);
      } else {
        setLoading(false);
      ...
      <View style={styles.content}>
        ...
        >
          ...
          <Text variant="bodyLarge" style={{ fontWeight: "bold" }}>
            {email ? email : "⚠️ EMAIL HILANG / UNDEFINED"}
          </Text>
        </View>
